...
TikTok provides access to a Commercial Content Library for the UK. This
is a GUI for its repository of all ads that are running on TikTok
including ads that are not presently active or paused by the advertisers
([[TikTok, n.d.]{.underline}](https://library.tiktok.com/)) However
...
Moreover, the functionality of the API could not be confirmed at the
...
to lack of access to the API at the time of this assessment. The GUI
also provides dates and country information in a locale-neutral manner.
...
the business name and business id. However, this has not been confirmed
...
from the platform via the GUI. However, in some observed instances, the
funder's name was not provided; however a string of numbers was instead
provided, appearing to be a business ID instead of a company's name
(e.g., An ad example [[linked
here]{.underline}](https://library.tiktok.com/ads/detail/?ad_id=1852856461089793)).
There is no way to find out which company corresponds to this ID using
the search function available on the GUI.
...
According to the API documentation, the API does not provide data
regarding whether an advertiser is verified or not.
...
The API and GUI documentations are provided in English, the official
language of the UK.
...
viewed creators from a specified category or whether the user is grouped
into a list of interests specified by the advertiser. However, this has
not been confirmed due to lack of access to the API at the time of this
assessment.
...
However, this information is not available in the API documentation and
this cannot be verified through the API due to lack of access at the
