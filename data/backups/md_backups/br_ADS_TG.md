...
[<span class="underline">https://core.telegram.org/api/sponsored-messages\#:\~:text=T%3D0%20%3A%20the%20user%20starts,The%20user%20resumes%20the%20video</span>](https://core.telegram.org/api/sponsored-messages#:~:text=T%3D0%20%3A%20the%20user%20starts,The%20user%20resumes%20the%20video)
...
Telegram provides an API for third-party apps to display sponsored
messages that replicate Telegram functionality. However, the API
endpoints do not provide access to a comprehensive ad database, only
allowing data collection from channels and groups known in advance

[<span class="underline">https://core.telegram.org/api/sponsored-messages</span>](https://core.telegram.org/api/sponsored-messages)
...
Telegram offers the Telegram Ad Platform (ads.telegram.org) as a tool
for creating and managing sponsored messages in public channels,
allowing advertisers to manage their ads and budgets and monitor
performance. However, this is not a public ad repository for extracting
data about all advertising content promoted on the platform.

[<span class="underline">https://ads.telegram.org/</span>](https://ads.telegram.org/)
...
No inactive ads can be extracted by the Telegram API. Their API is
intended to replicate the core functionality of a Telegram client. Thus,
the ad-related endpoints return only active ads in known channels so
that the Telegram clients can show them to their users.
...
No (?) The ads related endpoints lack a parameter for sponsored
messages, bots or channels ID.\<br\>The msg\_id parameter from
GetSponsoredMessagesRequest method is for a non-ad message. (???
confirmar)
